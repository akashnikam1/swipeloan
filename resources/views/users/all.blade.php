    ...
    <script type="text/javascript">
        ...
        function datatable1() {
            ...
            NioApp.DataTable('#myTable', {
                ...
                ajax: "{{ url('users/all') }}?loan_status=" + loan_status,
                "order": [
                    // [0, "desc"]
                ],
                responsive: !0,
                ...
                "columns": [
                    {
                        ...
                        render: function(data, type, row) {
                            if(row.phone_number) {
                                return '<a href="{{ url('users/user_details') }}/' + row.id + '"><span>' + data + '</span></a>';
                            } else {
                                return '<a href="{{ url('users/user_details') }}/' + row.id + '"><span>NA</span></a>';
                            }
                        }
                    },
                    {
        ...
        function changeNotificationStatus(id, checkbox) {
            if ($.trim(id)) {
                ...
                $.ajax({
                    ...
                    url: "{{ url('users/change-notification-status') }}",
                    type: "POST",
                    data: {
                        'id': id,
                        'status': status
